...
Template.researchForm.onCreated(function() {
  ...
  if (FlowRouter.current().route.name.startsWith("edit") || FlowRouter.current().route.name.startsWith("translate")) {
    ...
    this.subscribe("research.item", FlowRouter.getParam("slug"), () => {
      ...
      if (research && research.pdfId) this.pdfId.set(research.pdfId);
    });
  }
});
...
Template.researchForm.helpers({
  ...
  pdf: () => {
    // Pdf uploaded using the old method
    if (FlowRouter.current().route.name.startsWith("edit") || FlowRouter.current().route.name.startsWith("translate")) {
  ...
  uploaderOptions: () => {
    ...
    return {
      ...
      files:
        research && research.pdfId === instance.pdfId.get() ? ResearchFiles.find({ _id: research.pdfId }).fetch() : [],
      showFileList: true,
      showDelete: true,
      single: true,
