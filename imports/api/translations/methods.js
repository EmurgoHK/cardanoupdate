	...
	sendDailyPRRequest: async () => {
		...
		if (translations.length) {
			...
			gitCommitPush({
			    ...
			    token: process.env.GITHUB_API_TOKEN,
			    commitMessage: 
`Problem: Cardanoupdate strings are not translated
...
`
				...
				octokit.pullRequests.create({
					owner: 'EmurgoHK', 
					repo: 'cardanoupdate',
					title: 'Problem: Cardanoupdate strings are not translated',
	...
	translationVote: function(translationId, type) {
        ...
        let mod = Meteor.users.findOne({
        	_id: Meteor.userId()
        })

        if (!mod || !mod.moderator) {
          	throw new Meteor.Error('Error.', 'messages.mod_only')
        }
